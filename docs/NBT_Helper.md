...
说明：
打印nbt对象
	模板bSortCompound为true则对compound进行排序，否则无顺序保证
	模板PrintFunc用于自动推导funcPrint可调用对象类型

	参数nRoot为任意NBT_Type或NBT_Node，自动构造为view类型，具体请参考[NBT_Node_View.hpp](NBT_Node_View.md)
	参数funcPrint为任意可调用打印对象，需接受cpp20的std::format样式格式化字符串，默认实现为[NBT_Print.hpp](NBT_Print.md)
...
计算nbt对象的hash
	...
	参数nRoot为任意NBT_Type或NBT_Node，自动构造为view类型，具体请参考[NBT_Node_View.hpp](NBT_Node_View.md)
	参数nbtHash为用于计算哈希的对象，一般使用一个seed进行初始化，具体请参考[NBT_Hash.hpp](NBT_Hash.md)
	参数funBefore与funAfter都是用于在hash前后添加其它数据的可调用对象，默认值请参考前面的DefaultFunc
返回计算的hash
```